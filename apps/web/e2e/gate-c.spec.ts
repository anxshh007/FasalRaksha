...
const scene = (name: string) => readFileSync(join(FIXTURES, `${name}.jpg`));
const dataUrl = (name: string) => `data:image/jpeg;base64,${scene(name).toString('base64')}`;
const SCENES = ['onion-lot', 'onion-dark', 'onion-far', 'wall'];
...
async function fakeCamera(page: Page, scenes: string[] = SCENES) {
...
const set = (page: Page, patch: Partial<Omit<FakeCamera, 'calls' | 'tracks'>>) => page.evaluate((p) => Object.assign(window.__camera, p), patch);
const calls = (page: Page) => page.evaluate(() => window.__camera.calls.length);
const allStopped = (page: Page) => page.evaluate(() => window.__camera.tracks.length > 0 && window.__camera.tracks.every((t) => t.readyState === 'ended'));
...
const photoFile = (name: string) => ({ name: `IMG_${name}.jpg`, mimeType: 'image/jpeg', buffer: scene(name) });
